...
configurable). A hanging hook must not hang an upload indefinitely.

**Ordering is deterministic:** sort by an explicit numeric `order` on the hook,
tie-broken by plugin id. Transform output feeds the next transform, so order is
observable and must be stable across restarts.

**Neither kind runs on a dedupe hit** — dedupe short-circuits before store, and
re-running transforms on content already stored would be wasted work.
...
**Auth:** `/assets/exists` is a batch hash oracle — it reveals whether a
specific image is in the library. Require an upload-scoped API key and
rate-limit it.
